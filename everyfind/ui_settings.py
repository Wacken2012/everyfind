...
class SettingsDialog(Gtk.Dialog):
    def __init__(self, parent: Gtk.Window, settings: Optional[dict] = None):
        super().__init__(title=_("Settings"), transient_for=parent, flags=0)
        self.set_default_size(600, 400)

        # Use provided settings or a copy of defaults
        if settings is None:
        ...
        # Directories to index: listbox + add/remove
        lbl_dirs = Gtk.Label(label=_("Directories to index:"), xalign=0)
        ...
        col = Gtk.TreeViewColumn(_("Path"), renderer, text=0)
        ...
        self.add_dir_button = Gtk.Button(label=_("Add…"))
        self.remove_dir_button = Gtk.Button(label=_("Remove"))
        ...
        # File type filters
        lbl_filters = Gtk.Label(label=_("File type filters (comma-separated):"), xalign=0)
        ...
        # Excluded paths
        lbl_exclude = Gtk.Label(label=_("Excluded paths (comma-separated):"), xalign=0)
        ...
        # Auto reindex
        self.auto_reindex_check = Gtk.CheckButton(label=_("Enable automatic reindex"))
        self.auto_reindex_check.set_active(bool(self.settings.get("auto_reindex", False)))
        grid.attach(self.auto_reindex_check, 0, 6, 2, 1)

        lbl_interval = Gtk.Label(label=_("Reindex interval (minutes):"), xalign=0)
        ...
        # Language selection
        lbl_lang = Gtk.Label(label=_("Language:"), xalign=0)
        ...
        self.lang_combo.append_text("fr")
        self.lang_combo.append_text("es")
        self.lang_combo.append_text("pl")
        cur_lang = self.settings.get("language", "system")
        # Map language to index: system=0, de=1, en=2, fr=3, es=4, pl=5
        lang_map = {"system": 0, "de": 1, "en": 2, "fr": 3, "es": 4, "pl": 5}
        self.lang_combo.set_active(lang_map.get(cur_lang, 0))
    ...
    def on_reset(self, button):
        ...
        lang_map = {"system": 0, "de": 1, "en": 2, "fr": 3, "es": 4, "pl": 5}
        self.lang_combo.set_active(lang_map.get(lang, 0))
